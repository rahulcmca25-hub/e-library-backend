...
\> The domain apps make it easier to maintain and extend the system as additional library functionality is added. The repository also contains modules for favourites, notifications and reservations.
...
\| \`borrowed\_at\` | Timestamp when borrowing was created |
\| \`due\_date\` | Expected return date/time |
\| \`returned\_at\` | Actual return timestamp, when returned |
...
The \`BorrowRecord\` model contains an \`is\_overdue\` property.
...
    and timezone.now() > self.due\_date
)
\`\`\`
...
2\. The current time has passed \`due\_date\`

**### Important distinction**

\`is\_overdue\` is a property that evaluates whether a borrowing has passed its due date. It should not automatically be interpreted as an automatic database status transition unless the current business logic explicitly performs that transition.
...
The current implementation integrates with OpenAI and uses the configured `gpt-5-mini` model through the AI service.
...
\`\`\`mermaid
erDiagram
    USER ||--o{ BORROW\_RECORD : creates
    BOOK ||--o{ BORROW\_RECORD : has
...
    BORROW\_RECORD {
        int id
        datetime borrowed\_at
        datetime due\_date
        datetime returned\_at
        string status
        decimal fine
...
This is more representative of a real library system than simply storing an \`is\_borrowed\` flag on a book.
...
For a production deployment, the system could be extended with additional infrastructure such as:
